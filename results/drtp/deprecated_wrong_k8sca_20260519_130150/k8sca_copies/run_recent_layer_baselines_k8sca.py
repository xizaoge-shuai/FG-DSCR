...
def lrscheduler_assign(case, args):
    ...
    for c in containers:
        ...
        if best is None:
            # fallback: choose first node that can run it
            feasible = [node_id(n) for n in nodes if can_run_on_node(c, n)]
            if not feasible:
                raise RuntimeError(f"No feasible node for container {c['cid']}")
            best = feasible[0]
    ...
    return assignment, total_prefetch_mb
...
def simulate(case, assignment, args, total_prefetch_mb=0.0):
    layer_sizes = case.get("layer_sizes_mb", {})
    containers = case.get("containers", [])
    nodes = case.get("nodes", [])
    c_by_id = {c["cid"]: c for c in containers}
...
def main():
    ...
    if args.algo == "lrscheduler":
        ...
        assignment, prefetch_mb = lrscheduler_assign(case, args)
    elif args.algo == "break":
        args.algo_name = "BREAK-inspired"
        assignment, prefetch_mb = break_assign(case, args)
    elif args.algo == "orr":
        args.algo_name = "ORR-inspired-LGreedy"
        assignment, prefetch_mb = orr_assign(case, args)
    else:
        raise ValueError(args.algo)

    result = simulate(case, assignment, args, total_prefetch_mb=prefetch_mb)
