...
class LASAReimpl:
    ...
    def assign_containers(self) -> Dict[str, List[str]]:
        ...
        while remaining:
            ...
            # K8s-CA mode:
            # 如果现有边缘节点没有任何可行节点，则触发 Cluster Autoscaler，
            # 不再非法塞入普通节点队列。
            if best is None:
                cid = sorted(remaining)[0]
                self.ca_triggered.append(cid)
    ...
    def simulate(self, ordered_queues: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        objective_base = 0.5 * act + 0.5 * ams
        ca_triggered = len(getattr(self, "ca_triggered", []))
        total_requested = len(self.containers)
        ...
        return {
            ...
            "ca_triggered_containers": list(getattr(self, "ca_triggered", [])),
        }
