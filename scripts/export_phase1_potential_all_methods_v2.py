...
def frag_node(demand, cap):
    ...
    m = (pcpu + pmem + pdisk) / 3.0
    return (abs(pcpu-m) + abs(pmem-m) + abs(pdisk-m)) / 3.0
...
def extract_assignment_recursive(obj):
    if isinstance(obj, dict):
        for key in [
            "assignment", "assignments", "ordered_queues", "queues",
            "node_queues", "node_to_containers", "placement_by_node",
            "placements_by_node"
        ]:
            if key in obj and dict_looks_assignment(obj[key]):
        ...
        for v in obj.values():
            got = extract_assignment_recursive(v)
            if got:
                return got
...
def align_assignment(assignment, nodes):
    ...
    for k, cids in assignment.items():
        ...
        try:
            ...
            if 0 <= idx < len(node_list):
                out[node_list[idx]].extend(cids)
                continue
            if 1 <= idx <= len(node_list):
                out[node_list[idx-1]].extend(cids)
                continue
        except Exception:
            pass
    return dict(out)

def compute(case, result):
    layer_sizes = case.get("layer_sizes_mb", case.get("layer_sizes", {}))
    containers = case["containers"]

    nodes = {}
    for i, node in enumerate(case["nodes"]):
        nodes[get_node_id(node, i)] = node

    assignment = extract_assignment_recursive(result)
    if not assignment:
        raise RuntimeError("No assignment found in result json.")
    assignment = align_assignment(assignment, nodes)
    ...
    for t, c in enumerate(containers, start=1):
        ...
        r = get_res(c)
        node_demand[nid]["cpu"] += r["cpu"]
        node_demand[nid]["mem"] += r["mem"]
        node_demand[nid]["disk"] += r["disk"]
        ...
        for u, node in nodes.items():
            raw_delay += (node_download[u] ** 2) / max(get_bw(node), 1e-9)
    ...
    if not rows:
        raise RuntimeError("No matched containers.")
    return rows

def safe(s):
    return s.replace(" ", "_").replace("-", "_").replace("+", "plus").replace("/", "_")
...
def main():
    ...
    for N in REQS:
        ...
        for method, paths in METHODS:
            result_path = find_result(paths, N)
            ...
            try:
                ...
                hist = compute(case, result)
                last = hist[-1]

                method_dir = os.path.join(OUTDIR, safe(method))
                os.makedirs(method_dir, exist_ok=True)
                csv_path = os.path.join(method_dir, f"potential_{safe(method)}_{N}.csv")
                ...
                print("[FAIL]", N, method, e)

    summary = os.path.join(OUTDIR, "summary_potential_final_all_methods.md")
    with open(summary, "w", encoding="utf-8") as f:
        ...
        for r in sorted(final_rows, key=lambda x: (x["requests"], x["method"])):
            f.write("| {} | {} | {:.3f} | {:.3f} | {:.3f} | {:.3f} | {:.3f} |\n".format(
                r["requests"], r["method"], r["phi_total"], r["delay_term"],
                r["frag_term"], r["aff_reward_term"], r["load_term"]
            ))
        f.write(f"\nFailures: {len(failures)}\n")
        for N, m, reason in failures:
            f.write(f"\n[FAIL] N={N}, method={m}, reason={reason}")

    comps = ["phi_total", "delay_term", "frag_term", "aff_reward_term", "load_term"]
