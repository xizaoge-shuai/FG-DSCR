...
REQS=($(seq 600 "$STEP" 800))
METHODS=(ilrsa lrs gahrl orr lasa fg)

CASE_DIR=cases/drtp_cache_only_sweep_88_prefix
OUT_DIR=results/drtp/k8s_same_scale/fig5_overall_prefix_fine_600_800_step${STEP}
LOG_DIR=results/drtp/k8s_same_scale/logs/fig5_overall_prefix_fine_600_800_step${STEP}
...
do
  ...
  do
    ...
    echo "[FINE-PREFIX] method=$M req=$REQ"
    bash scripts/k8s_same_scale/run_one.sh "$M" "$CASE" "$OUT" > "$LOG" 2>&1
  done
done

echo "[DONE] fine prefix 600-800 step=$STEP"
