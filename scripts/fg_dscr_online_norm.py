            ...
            for st in beam:
                ...
                if getattr(self, "online_norm", True):
                    ...
                    for row in candidate_rows:
                        # Weighted online-normalized dynamic gain.
                        # The alpha weights are used after component-wise online normalization.
                        denom = (
                            self.alpha1_reuse
                            + self.alpha2_future
                            + self.alpha3_pull
                        ...
                        row["gain_used"] = (
                            self.alpha1_reuse  * row.get("reuse_norm", 0.5)
                            + self.alpha2_future * row.get("future_norm", 0.5)
                            + self.alpha3_pull   * row.get("pull_save_norm", 0.5)
                            + self.alpha4_evict  * row.get("evict_save_norm", 0.5)
                        ) / denom
                else:
                    for row in candidate_rows:
...
def main():
    ...
    parser.add_argument("--alpha1-reuse", type=float, default=1.0,
                        help="Weight of normalized Reuse term in online-normalized phase-2 gain.")
    parser.add_argument("--alpha2-future", type=float, default=1.0,
                        help="Weight of normalized Future term in online-normalized phase-2 gain.")
    parser.add_argument("--alpha3-pull", type=float, default=1.0,
                        help="Weight of normalized Pull-saving term in online-normalized phase-2 gain.")
    parser.add_argument("--alpha4-evict", type=float, default=1.0,
                        help="Weight of normalized Evict-saving term in online-normalized phase-2 gain.")
    ...
    scheduler = FGDscrScheduler(
        ...
        alpha1_reuse=args.alpha1_reuse,
        alpha2_future=args.alpha2_future,
        alpha3_pull=args.alpha3_pull,
        alpha4_evict=args.alpha4_evict,
