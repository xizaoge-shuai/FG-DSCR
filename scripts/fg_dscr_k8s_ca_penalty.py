...
class FGDscrScheduler:
    ...
    # LSPGDSF weights
        ...
        # CA / guarantee mechanism for unschedulable containers.
        # If no ordinary edge node satisfies hard resource feasibility,
        # the container is marked as served by a costly guarantee mechanism
        # instead of aborting the whole scheduling process.
        self.ca_eid = "__CA_GUARANTEE__"
        self.ca_fixed_penalty = 1000.0
    ...
    def feasible_assign(
        ...
        """
        K8s-style hard feasibility filter.

        A container can be assigned to a node only if the node still has enough
        remaining CPU / memory / disk resources after considering containers
        ...
        # repo_capacity_mb only controls reusable image-layer cache capacity,
        # not hard deployment feasibility.
        return True
    ...
    def node_layer_counts(
        ...
        for cid, eid in assignment.items():
            if eid not in cnts:
                # Containers served by CA / guarantee mechanism are not placed
                # on ordinary edge nodes and therefore do not contribute to
                # node-local layer counts.
                continue
    ...
    def greedy_init_assignment(self) -> Dict[str, str]:
        ...
        for cid in cids:
            ...
            if not feasible_eids:
                # No node can host this container under hard resource filtering.
                # In real K8s clusters, such a Pod would become unschedulable
                # and may trigger Cluster Autoscaler / extra resource provisioning.
                # We model this as a costly guarantee mechanism and continue.
                assignment[cid] = self.ca_eid
                continue
    ...
    def run(self) -> Dict:
        ...
        objective_without_ca_penalty = self.alpha_obj * ACT + (1.0 - self.alpha_obj) * AMS

        failed_deployments = len(ca_cids)
        total_requests = total_num + failed_deployments

        # Average CA / guarantee penalty. This keeps the penalty comparable
        # across different request scales.
        ca_penalty = self.ca_fixed_penalty * failed_deployments / max(total_requests, 1)
        objective = objective_without_ca_penalty + ca_penalty

        ca_downloaded_mb = sum(self.image_size(cid) for cid in ca_cids)
