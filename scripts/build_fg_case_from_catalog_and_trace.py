...
def main():
    ...
    parser.add_argument("--repo-capacity-mb", type=int, default=4096)
    parser.add_argument("--warm-cache-ratio", type=float, default=0.05)
    ...
    for n_containers in args.sizes:
        ...
        # 节点配置
        nodes = []
        repo_capacity_mb = int(args.repo_capacity_mb)

        warm_cap = max(0, int(repo_capacity_mb * args.warm_cache_ratio))
        if warm_cap > 0:
            hot_cache = greedy_hot_cache(dict(layer_freq), layer_sizes_mb, warm_cap)
        else:
            hot_cache = []
